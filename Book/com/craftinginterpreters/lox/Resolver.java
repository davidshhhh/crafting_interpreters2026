...
import Book.com.craftinginterpreters.lox.Expr.Variable;

class Resolver implements Expr.Visitor<Void>, Stmt.Visitor<Void> {
  private final Interpreter interpreter;
  private final Stack<Map<String, Boolean>> scopes = new Stack<>();
  private FunctionType currentFunction = FunctionType.NONE;
  private final Stack<Map<String, Variable>> scopes = new Stack<>(); // chqp 11 new code challeneg 3 
  ...
  scopeNextIndex.push(0); // Initialize index for new scope
}
  ...
  for (int i = scopes.size() - 1; i >= 0; i--) {
    if (scopes.get(i).containsKey(name.lexeme)) {
      ...
      // Pass both depth (distance) and the unique index to the interpreter
